...
class MySQLOrigin(Origin):
  def __init__(self, name: str, host: str, port: int = 3306, database: str = None, user: str = None, password: str = None, query: str = None):
    super().__init__()
    self.name = name
    self.host = host
    self.port = port
  ...
  def pump(self) -> None:
    try:
      ...
      print(f"MySQLOrigin '{self.name}' executing query...")
      ...
      print(f"MySQLOrigin '{self.name}' successfully executed query:")
      ...
      if len(self.outputs) > 0:
        ...
        print(f"Warning: MySQLOrigin '{self.name}' has no output pipe configured")
...
class MySQLDestination(Destination):
  def __init__(self, name: str, host: str, port: int = 3306, database: str = None, user: str = None, password: str = None, table: str = None, if_exists: str = 'append'):
    super().__init__()
    self.name = name
    self.host = host
    self.port = port
  ...
  def sink(self, data_package: DataPackage) -> None:
    ...
    try:
      ...
      print(f"MySQLDestination '{self.name}' writing data to MySQL...")
      print(f"  - Database: {self.database}")
      ...
      print(f"MySQLDestination '{self.name}' successfully wrote data:")
      print(f"  - Rows written: {len(df)}")
      print(f"  - Columns written: {len(df.columns)}")
      print(f"  - Table: {self.database}.{self.table}")
