...
class PostgresOrigin(Origin):
  ...
  before_query : str, optional
    ...
    Useful for: creating temp tables, calling functions, setting session variables.
    Default: None
  ...
  ...     table="customers",
  ...     after_query='''
  ...         INSERT INTO audit.extraction_log (table_name, extracted_at, record_count)
  ...
  ...         SELECT validate_sales_data();
  ...     ''',
  ...     query="SELECT * FROM staging_sales",
  ...     after_query='''
  ...         -- Log extraction
  ...         INSERT INTO audit.etl_log (pipeline, run_at, records)
  ...         VALUES ('daily_sales', NOW(), (SELECT COUNT(*) FROM staging_sales));
  ...         
  ...         -- Cleanup
  ...
  def __init__(
    ...
    if query and table:
      raise ValueError(f"PostgresOrigin '{self.name}': cannot specify both 'query' and 'table'. Use one or the other.")
  ...
  def _initialize_engine(self):
    ...
    try:
      ...
      # Configurar connect_args con timeout si se especificó
      connect_args = {}
      if self.timeout:
  ...
  def _execute_query(self, sql_query: str, description: str) -> None:
    ...
    """
    try:
      print(f"\nPostgresOrigin '{self.name}' executing {description}...")
      ...
      with self.engine.connect() as connection:
        ...
        print(f"✅ PostgresOrigin '{self.name}' {description} executed successfully")
      ...
      print(f"❌ Error: PostgresOrigin '{self.name}' failed to execute {description}: {str(e)}")
  ...
  def _build_query(self) -> str:
    ...
    if self.table:
      ...
      # Soporta formatos: 'table' o 'schema.table'
      parts = self.table.split('.')
      
      if len(parts) == 1:
        # Solo nombre de tabla (usa schema por defecto)
        ...
        raise ValueError(f"PostgresOrigin '{self.name}': invalid table format '{self.table}'. Use 'table' or 'schema.table'")
  ...
  def pump(self) -> None:
    ...
    Execution flow:
    1. Initialize PostgreSQL engine
    ...
    try:
      ...
      print(f"\n{'='*70}")
      print(f"PostgresOrigin '{self.name}' executing MAIN extraction query...")
      print(f"{'='*70}")
      ...
      print(f"\n{'='*70}")
      print(f"PostgresOrigin '{self.name}' MAIN query results:")
      print(f"{'='*70}")
      print(f"  📊 Results:")
      ...
      if len(self.outputs) > 0:
        ...
        print(f"\n{'='*70}")
        print(f"⚠️  Warning: PostgresOrigin '{self.name}' has no output pipe configured")
        print(f"{'='*70}")
      ...
      if "could not connect" in error_msg or "connection refused" in error_msg:
        ...
        print(f"❌ Error: PostgresOrigin '{self.name}' query timeout: {str(e)}")
...
class PostgresDestination(Destination):
  ...
  after_query : str, optional
    ...
    Useful for: audit logging, post-processing, refreshing views, cleanup.
    Default: None
  timeout : float, optional
    Query timeout in seconds (default: None = no timeout)
  ...
  ...     schema="reports",
  ...     before_query='''
  ...         TRUNCATE TABLE reports.daily_summary;
  ...     ''',
  ...     if_exists="append"
  ... )
  ...
  ...         VALUES ('customers', NOW(), (SELECT COUNT(*) FROM public.customers));
  ...     '''
  ... )
  
  # Example 4: Complete workflow with before and after
  >>> dest = PostgresDestination(
  ...     name="orders_etl",
  ...     host="localhost",
  ...     database="warehouse",
  ...
  ...     after_query='''
  ...         -- Validate data
  ...         SELECT validate_orders();
  ...         
  ...         -- Refresh materialized views
  ...         REFRESH MATERIALIZED VIEW reports.orders_summary;
  ...         
  ...         -- Log success
  ...
  def _execute_query(self, sql_query: str, description: str) -> None:
    ...
    """
    try:
      print(f"\nPostgresDestination '{self.name}' executing {description}...")
      print(f"  Query preview: {sql_query[:200]}{'...' if len(sql_query) > 200 else ''}")
      ...
      with self.engine.connect() as connection:
        ...
        print(f"✅ PostgresDestination '{self.name}' {description} executed successfully")
      ...
      print(f"❌ Error: PostgresDestination '{self.name}' failed to execute {description}: {str(e)}")
      print(f"Query: {sql_query[:500]}{'...' if len(sql_query) > 500 else ''}")
      raise
  ...
  def sink(self, data_package: DataPackage) -> None:
    ...
    try:
      ...
      print(f"\n{'='*70}")
      print(f"PostgresDestination '{self.name}' loading data to PostgreSQL...")
      print(f"{'='*70}")
      print(f"  - Database: {self.database}")
      print(f"  - Schema: {self.schema}")
      print(f"  - Table: {self.table}")
      print(f"  - Full table reference: {table_ref}")
      print(f"  - If exists: {self.if_exists}")
      print(f"  - DataFrame shape: {df.shape}")
      print(f"  - DataFrame columns: {list(df.columns)}")
      
      print(f"\nPostgresDestination '{self.name}' starting load operation...")
      start_time = time.time()
      ...
      print(f"\n{'='*70}")
      print(f"PostgresDestination '{self.name}' LOAD completed successfully:")
      print(f"{'='*70}")
      ...
      print(f"\n{'='*70}")
      print(f"✅ PostgresDestination '{self.name}' completed successfully")
      print(f"{'='*70}")
      ...
      if "could not connect" in error_msg or "connection refused" in error_msg:
        ...
        print(f"❌ Error: PostgresDestination '{self.name}' query timeout: {str(e)}")
        print(f"Timeout was: {self.timeout}s")
      else:
