...
class OpenAIPromptTransformer(Node):
  def __init__(self, name: str, model: str, api_key: str, prompt: str, max_tokens: int = 16000):
    ...
    if not model or not model.strip():
      raise ValueError(f"OpenAIPromptTransformer '{self.name}': model cannot be empty")
    
    # Validar que api_key no esté vacía
    if not api_key or not api_key.strip():
      raise ValueError(f"OpenAIPromptTransformer '{self.name}': api_key cannot be empty")
    
    # Validar que prompt no esté vacío
    if not prompt or not prompt.strip():
      raise ValueError(f"OpenAIPromptTransformer '{self.name}': prompt cannot be empty")
    
    # Validar que max_tokens sea positivo
    if max_tokens <= 0:
      raise ValueError(f"OpenAIPromptTransformer '{self.name}': max_tokens must be positive, got {max_tokens}")
  ...
  def add_input_pipe(self, pipe: Pipe) -> None:
    ...
    if len(self.inputs.keys()) == 0:
      ...
      raise ValueError(f"OpenAIPromptTransformer '{self.name}' can only have 1 input")
  ...
  def add_output_pipe(self, pipe: Pipe) -> Pipe:
    ...
    if len(self.outputs.keys()) == 0:
      ...
      raise ValueError(f"OpenAIPromptTransformer '{self.name}' can only have 1 output")
  
  def _initialize_client(self):
    """
    Inicializa el cliente de OpenAI usando OpenAI SDK
    """
    try:
      self.client = OpenAI(api_key=self.api_key)
      print(f"OpenAIPromptTransformer '{self.name}' OpenAI client initialized successfully")
    except Exception as e:
      raise ValueError(f"OpenAIPromptTransformer '{self.name}' failed to initialize OpenAI client: {str(e)}")
  
  def sink(self, data_package: DataPackage) -> None:
    print(f"OpenAIPromptTransformer '{self.name}' received data from pipe: '{data_package.get_pipe_name()}'")
    df = data_package.get_df()
    
    # Almacenar el DataFrame para procesamiento en pump()
    self.received_df = df
    print(f"OpenAIPromptTransformer '{self.name}' stored DataFrame with {len(df)} rows and {len(df.columns)} columns")
  ...
  def pump(self) -> None:
    if self.received_df is None:
      print(f"Warning: OpenAIPromptTransformer '{self.name}' has no data to process")
      return
    
    if len(self.outputs) == 0:
      print(f"Warning: OpenAIPromptTransformer '{self.name}' has no output pipe configured")
      return
    
    df = self.received_df
    print(f"OpenAIPromptTransformer '{self.name}' processing DataFrame with {len(df)} rows")
    print(f"OpenAIPromptTransformer '{self.name}' using model: {self.model}")
    ...
    try:
      ...
      print(f"OpenAIPromptTransformer '{self.name}' sending request to OpenAI API...")
...
Remember: Return ONLY raw CSV format, no explanations, no markdown, no code blocks."""
      ...
      # Llamar a la API de OpenAI usando OpenAI SDK
      ...
      print(f"OpenAIPromptTransformer '{self.name}' received response from OpenAI")
      ...
      try:
        ...
        print(f"Error: OpenAIPromptTransformer '{self.name}' failed to parse CSV response: {str(e)}")
        print(f"Response preview (first 500 chars): {response_text[:500]}")
        print(f"Response ending (last 500 chars): ...{response_text[-500:]}")
        raise
      
      print(f"OpenAIPromptTransformer '{self.name}' completed transformation:")
      ...
      print(f"OpenAIPromptTransformer '{self.name}' pumped transformed data through pipe '{output_pipe.get_name()}'")
      
    except Exception as e:
      print(f"Error: OpenAIPromptTransformer '{self.name}' unexpected error: {str(e)}")
