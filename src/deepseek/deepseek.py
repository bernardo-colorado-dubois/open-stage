...
class DeepSeekPromptTransformer(Node):
  def __init__(self, name: str, model: str, api_key: str, prompt: str, max_tokens: int = 8192):
    super().__init__()
    self.name = name
    self.model = model
    ...
    self.base_url = "https://api.deepseek.com"  # Configuración interna
    self.received_df = None
    self.client = None
    
    # Validar que model no esté vacío
    if not model or not model.strip():
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}': model cannot be empty")
    
    # Validar que api_key no esté vacía
    if not api_key or not api_key.strip():
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}': api_key cannot be empty")
    
    # Validar que prompt no esté vacío
    if not prompt or not prompt.strip():
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}': prompt cannot be empty")
    
    # Validar que max_tokens esté en el rango permitido por DeepSeek [1, 8192]
    if max_tokens <= 0 or max_tokens > 8192:
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}': max_tokens must be between 1 and 8192, got {max_tokens}")
  ...
  def add_input_pipe(self, pipe: Pipe) -> None:
    ...
    if len(self.inputs.keys()) == 0:
      ...
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}' can only have 1 input")
  ...
  def add_output_pipe(self, pipe: Pipe) -> Pipe:
    ...
    if len(self.outputs.keys()) == 0:
      ...
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}' can only have 1 output")
  ...
  def _initialize_client(self):
    ...
    try:
      ...
      print(f"DeepSeekPromptTransformer '{self.name}' DeepSeek client initialized successfully")
      print(f"Base URL: {self.base_url}")
    except Exception as e:
      raise ValueError(f"DeepSeekPromptTransformer '{self.name}' failed to initialize DeepSeek client: {str(e)}")
  
  def sink(self, data_package: DataPackage) -> None:
    print(f"DeepSeekPromptTransformer '{self.name}' received data from pipe: '{data_package.get_pipe_name()}'")
    df = data_package.get_df()
    
    # Almacenar el DataFrame para procesamiento en pump()
    self.received_df = df
    print(f"DeepSeekPromptTransformer '{self.name}' stored DataFrame with {len(df)} rows and {len(df.columns)} columns")
  ...
  def pump(self) -> None:
    if self.received_df is None:
      print(f"Warning: DeepSeekPromptTransformer '{self.name}' has no data to process")
      return
    
    if len(self.outputs) == 0:
      print(f"Warning: DeepSeekPromptTransformer '{self.name}' has no output pipe configured")
      return
    
    df = self.received_df
    print(f"DeepSeekPromptTransformer '{self.name}' processing DataFrame with {len(df)} rows")
    print(f"DeepSeekPromptTransformer '{self.name}' using model: {self.model}")
    ...
    try:
      ...
      print(f"DeepSeekPromptTransformer '{self.name}' sending request to DeepSeek API...")
...
Remember: Return ONLY raw CSV format, no explanations, no markdown, no code blocks."""
      ...
      print(f"DeepSeekPromptTransformer '{self.name}' received response from DeepSeek")
      ...
      try:
        ...
        print(f"Error: DeepSeekPromptTransformer '{self.name}' failed to parse CSV response: {str(e)}")
        print(f"Response preview (first 500 chars): {response_text[:500]}")
        print(f"Response ending (last 500 chars): ...{response_text[-500:]}")
        raise
      
      print(f"DeepSeekPromptTransformer '{self.name}' completed transformation:")
      ...
      print(f"DeepSeekPromptTransformer '{self.name}' pumped transformed data through pipe '{output_pipe.get_name()}'")
      
    except Exception as e:
      print(f"Error: DeepSeekPromptTransformer '{self.name}' unexpected error: {str(e)}")
